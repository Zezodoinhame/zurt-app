...
import { formatCurrency, getUsageColor } from '../../utils/formatters';
...
export default function CardDetailPanel({ card, hideValues }: Props) {
  ...
  const display = (value: number) =>
    hideValues ? `${currency === 'BRL' ? 'R$' : '$'} \u2022\u2022\u2022\u2022\u2022\u2022` : formatCurrency(value, currency);
    ...
    <View style={styles.container}>
      ...
      <View style={styles.row}>
        <View style={styles.col}>
          <Text style={styles.label}>{t('cards.currentInvoice')}</Text>
          <Text style={styles.valueDark}>{display(card.faturaAtual)}</Text>
        </View>
        <View style={[styles.col, { alignItems: 'flex-end' }]}>
          <Text style={styles.label}>{t('cards.available')}</Text>
      ...
      <View style={styles.usageSection}>
        <View style={styles.usageLabels}>
          <Text style={styles.usageText}>{(pctUsado * 100).toFixed(0)}% {t('cards.used')}</Text>
          <Text style={styles.usageText}>{t('cards.limit')} {hideValues ? '\u2022\u2022\u2022' : formatCurrency(card.limiteTotal, currency)}</Text>
        </View>
        <View style={styles.usageBarBg}>
          <View style={[styles.usageBarFill, { width: `${Math.min(pctUsado * 100, 100)}%`, backgroundColor: barColor }]} />
        </View>
      </View>
      ...
      <View style={styles.actions}>
        <TouchableOpacity style={styles.btnSecondary} activeOpacity={0.7}>
          ...
          <Text style={styles.btnSecondaryText}>{t('cards.viewInvoice')}</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.btnPrimary} activeOpacity={0.7}>
          <Ionicons name="card-outline" size={16} color="#fff" />
          <Text style={styles.btnPrimaryText}>{t('cards.pay')}</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
