...
export default function SpendingCategories({ categories, hideValues }: Props) {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>{t('cards.spendingByCategory')}</Text>
      <Text style={styles.subtitle}>
        {t('cards.totalSpending')}: {hideValues ? `${currency === 'BRL' ? 'R$' : '$'} \u2022\u2022\u2022\u2022\u2022\u2022` : formatCurrency(total, currency)}
      </Text>
        ...
        >
          <View style={styles.catLeft}>
            ...
            <View>
              ...
              <Text style={styles.catPct}>{(cat.pct * 100).toFixed(1)}% {t('cards.ofTotal')}</Text>
            </View>
          </View>
          <Text style={styles.catValor}>
            {hideValues ? `${currency === 'BRL' ? 'R$' : '$'} \u2022\u2022\u2022` : formatCurrency(cat.valor, currency)}
          </Text>
        </View>
      ))}
    </View>
