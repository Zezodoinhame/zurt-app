...
import ZurtLoading from '../../src/components/shared/ZurtLoading';
...
// ---------------------------------------------------------------------------
// HeroSparkline â€” lightweight SVG sparkline for the hero card
// ---------------------------------------------------------------------------
  ...
  return (
    <View style={[styles.screen, { paddingTop: insets.top + 24 }]}>
      ...
      >
        ...
        {/* ---------------------------------------------------------------- */}
        {isLoading ? (
          <ZurtLoading message="Atualizando mercado..." />
        ) : error && !summary && !error.includes("Unauthorized") && !error.includes("401") ? (
          ...
          <>
            ...
            <View style={styles.quickActions}>
              {([
                ...
                { icon: 'gift' as const, label: 'Convidar\namigos', accent: false, onPress: () => Share.share({ message: 'ConheÃ§a o ZURT - inteligÃªncia patrimonial na palma da mÃ£o! https://zurt.com.br' }) },
              ] as const).map((btn, i) => (
                <TouchableOpacity
                  key={i}
              ...
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10 }}>
                ...
                {ibovespa && (
                  <TouchableOpacity
                    ...
                    activeOpacity={0.7}
                    style={{
                      flex: 1, minWidth: '46%', backgroundColor: colors.card,
                      borderRadius: 14, borderWidth: 1, borderColor: colors.border,
                      padding: 14, borderLeftWidth: 3, borderLeftColor: '#8B5CF6',
                    }}
                  >
                    <Text style={{ fontSize: 11, fontWeight: '700', color: colors.text.muted, textTransform: 'uppercase', letterSpacing: 0.5 }}>IBOV</Text>
                    <Text style={{ fontSize: 20, fontWeight: '800', color: colors.text.primary, marginTop: 4 }}>
                      {Number(ibovespa.regularMarketPrice || 0) >= 1000
                        ? `${(Number(ibovespa.regularMarketPrice) / 1000).toFixed(1)}k`
                        : Number(ibovespa.regularMarketPrice || 0).toFixed(0)}
                    </Text>
                    <Text style={{ fontSize: 12, fontWeight: '600', marginTop: 2, color: Number(ibovespa.regularMarketChangePercent || 0) >= 0 ? '#00D4AA' : '#FF4D4D' }}>
                      {Number(ibovespa.regularMarketChangePercent || 0) >= 0 ? '+' : ''}{Number(ibovespa.regularMarketChangePercent || 0).toFixed(2)}%
                    </Text>
                  </TouchableOpacity>
                )}
                ...
                {usdBrl && (
                  ...
                  >
                    <Text style={{ fontSize: 11, fontWeight: '700', color: colors.text.muted, textTransform: 'uppercase', letterSpacing: 0.5 }}>USD/BRL</Text>
                    <Text style={{ fontSize: 20, fontWeight: '800', color: colors.text.primary, marginTop: 4 }}>
                      R$ {Number(usdBrl.bidPrice || 0).toFixed(2)}
                    </Text>
                    <Text style={{ fontSize: 12, fontWeight: '600', marginTop: 2, color: Number(usdBrl.regularMarketChangePercent || 0) >= 0 ? '#00D4AA' : '#FF4D4D' }}>
                      {Number(usdBrl.regularMarketChangePercent || 0) >= 0 ? '+' : ''}{Number(usdBrl.regularMarketChangePercent || 0).toFixed(2)}%
                    </Text>
                  </TouchableOpacity>
                )}
                ...
                {eurBrl && (
                  ...
                  >
                    <Text style={{ fontSize: 11, fontWeight: '700', color: colors.text.muted, textTransform: 'uppercase', letterSpacing: 0.5 }}>EUR/BRL</Text>
                    <Text style={{ fontSize: 20, fontWeight: '800', color: colors.text.primary, marginTop: 4 }}>
                      R$ {Number(eurBrl.bidPrice || 0).toFixed(2)}
                    </Text>
                    <Text style={{ fontSize: 12, fontWeight: '600', marginTop: 2, color: Number(eurBrl.regularMarketChangePercent || 0) >= 0 ? '#00D4AA' : '#FF4D4D' }}>
                      {Number(eurBrl.regularMarketChangePercent || 0) >= 0 ? '+' : ''}{Number(eurBrl.regularMarketChangePercent || 0).toFixed(2)}%
                    </Text>
                  </TouchableOpacity>
                )}
                ...
                {btcBrl && (
                  <TouchableOpacity
                    ...
                    style={{
                      flex: 1, minWidth: '46%', backgroundColor: colors.card,
                      borderRadius: 14, borderWidth: 1, borderColor: colors.border,
                      padding: 14, borderLeftWidth: 3, borderLeftColor: '#FF8C42',
                    }}
                  >
                    <Text style={{ fontSize: 11, fontWeight: '700', color: colors.text.muted, textTransform: 'uppercase', letterSpacing: 0.5 }}>BTC</Text>
                    <Text style={{ fontSize: 20, fontWeight: '800', color: colors.text.primary, marginTop: 4 }}>
                      {`R$ ${(Number(btcBrl.regularMarketPrice || 0) / 1000).toFixed(0)}k`}
                    </Text>
                    <Text style={{ fontSize: 12, fontWeight: '600', marginTop: 2, color: Number(btcBrl.regularMarketChangePercent || 0) >= 0 ? '#00D4AA' : '#FF4D4D' }}>
                      {Number(btcBrl.regularMarketChangePercent || 0) >= 0 ? '+' : ''}{Number(btcBrl.regularMarketChangePercent || 0).toFixed(2)}%
                    </Text>
                  </TouchableOpacity>
                )}
              </View>
